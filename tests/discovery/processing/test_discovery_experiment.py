...
def test_filter_apriori_thresholds():
    experiment = DiscoveryExperiment(
        ...
        apriori_columns=['u', 'v'], apriori_thresholds=[3, 6], apriori_weights=[1, 1], apriori_max_or_min=['max', 'min']
    )
    ...
    assert result['u'].values.all() == np.array([1, 4, 5, 7, 8]).all()
    assert result['v'].values.all() == np.array([1, 4, 5, 7, 8]).all()
    assert result['x'].values.all() == np.array([1, 4, 5, 7, 8]).all()
    assert result['y'].values.all() == np.array([1, 4, 5, 7, 8]).all()
