...
def test_decide_max_or_min_full():
    df = pd.DataFrame()
    df['u'] = [1, 2, 3, 4, 5, 6, 7, 8]
    df['v'] = [1, 2, 3, 4, 5, 6, 7, 8]
    df['x'] = [np.nan, np.nan, np.nan, 4, np.nan, np.nan, 7, 8]
    ...
    assert np.array_equal(result['v'].values, np.array([-1, -2, -3, -4, -5, -6, -7, -8]))
    assert np.array_equal(np.nan_to_num(result['x'].values), np.array([0, 0, 0, -4, 0, 0, -7, -8]))
    assert np.array_equal(np.nan_to_num(result['y'].values), np.array([1, 0, 0, 0, 0, 0, 0, 8]))


def test_decide_max_or_min_empty():
    df = pd.DataFrame()
