...
def run_experiment(dataset):
    request_body = json.loads(request.data)
    dataframe, plot_url = DiscoveryService.run_experiment(dataset, request_body)
    html_dataframe = dataframe.to_html(index=False,
                                       table_id='formulations_dataframe',
                                       classes='table table-bordered table-striped table-hover')

    body = {'template': render_template('experiment_result.html', df=html_dataframe, plot_url=plot_url)}
    return make_response(jsonify(body), 200)
