...
describe("Test blending three custom materials with properties with negative values", () => {
  ...
  it("Create blend from three custom materials", () => {
    ...
    cy.findByText("CO₂ footprint (kg): -4.4", { exact: false }).should("exist");
    ...
    cy.findByText("CO₂ footprint (kg): -3.65", { exact: false }).should("exist");
    ...
    cy.findByText("CO₂ footprint (kg): 2.1", { exact: false }).should("exist");
    cy.findAllByText("Delivery time (days): 20.0", { exact: false }).eq(1).should("exist");
    ...
    cy.findByText("CO₂ footprint (kg): 2.85", { exact: false }).should("exist");
...
describe("Test autocorrect features", () => {
  ...
  it("Increment, min and max fields are corrected automatically", () => {
    ...
    // Check that it clips the value to 0.01 from below
    cy.findAllByLabelText("Increment (%)").first().type("{moveToStart}-");
    cy.findAllByLabelText("Increment (%)").first().should("have.value", 0.01);
    // Check that it rounds the value 0 to 0.01
    cy.findAllByLabelText("Increment (%)").first().clear().type("0");
    cy.findAllByLabelText("Increment (%)").first().should("have.value", 0.01);
